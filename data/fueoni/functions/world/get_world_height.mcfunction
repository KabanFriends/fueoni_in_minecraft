...
execute if score in_world foGameData matches 0 run tellraw @a[advancements={fueoni:spawn_egg/start_game=true}] [{"text":""},{"translate":"fueoni.message.prefix","fallback":"増え鬼","color":"yellow","bold":true},{"text":" » ","color":"gray","bold":false},{"translate":"fueoni.message.cannot_detect_world_height","fallback":"ワールドの高さを検出できないため、既定値を使用します。","color":"red"}]
